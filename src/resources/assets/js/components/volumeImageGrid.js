...
biigle.$component('volumes.components.volumeImageGrid', {
    ...
    template: '<div class="image-grid" @wheel.prevent="scroll">' +
        '<div class="image-grid__images" ref="images">' +
            '<image-grid-image v-for="image in displayedImages" :key="image.id" :image="image" :empty-url="emptyUrl" :selected-label="selectedLabel" :selectable="selectable" @select="emitSelect"></image-grid-image>' +
        '</div>' +
