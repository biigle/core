 ...
 * @description Service managing the image sorting of the volume index page
 */
angular.module('biigle.volumes').service('sort', function (VOLUME_ID, VOLUME_IMAGES) {
        "use strict";

        var sorterLocalStorageKey = 'biigle.volumes.' + VOLUME_ID + '.sorting.sorter';
        var sequenceLocalStorageKey = 'biigle.volumes.' + VOLUME_ID + '.sorting.sequence';
        var directionLocalStorageKey = 'biigle.volumes.' + VOLUME_ID + '.sorting.direction';
        ...
        this.activateSorter = function (newSorter, newSequence) {
            ...
            if (newSequence.length !== DEFAULTS.SEQUENCE.length) {
                throw 'Requested sorting sequence length does not match the number of images in the volume!';
            }
