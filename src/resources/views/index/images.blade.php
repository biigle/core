<div class="volume__images" data-ng-controller="ImagesController" data-ng-class="getClass()">
    <figure class="volume-figure ng-cloak" data-volume-figure="" data-ng-repeat="id in getImageIds()" data-ng-class="getClass()" data-popover-placement="@{{getPopoverPlacement()}}" data-popover-trigger="none" data-popover-is-open="isPopoverOpen()" data-uib-popover-template="'imageLabelPopover.html'">
        @if (Route::has('annotate'))
            <a class="volume-figure__link" href="{{ route('annotate', '') }}/@{{id}}" title="@{{isInLabelMode() ? 'Attach the selected label' : 'Annotate this image'}}" data-ng-click="handleClick($event)">
        @else
            <div title="@{{isInLabelMode() ? 'Attach the selected label' : ''}}" data-ng-click="handleClick($event)">
        @endif
            <div class="volume-figure__flags" data-ng-show="hasFlag()">
