<div class="sidebar__foldout" data-ng-class="{open:(foldout=='categories')}" data-ng-controller="CategoriesController">
    <div class="categories-search">
        <input class="form-control" type="text" data-ng-model="searchCategory" data-uib-typeahead="category.name for category in categories | filter:$viewValue | limitTo:10" data-typeahead-on-select="selectItem($item)" placeholder="Find label" />
    ...
    <div class="categories-body">
        ...
        <ol class="categories-list__items list-unstyled">
            <li class="label-category-item clearfix" data-ng-class="getClass()" data-ng-repeat="item in favourites" title="Select favourite @{{hotkeysMap[$index]}}"></li>
        </ol>
        </div>
...
<script type="text/ng-template" id="label-item.html">
    <div class="item clearfix" data-ng-click="selectItem(item)">
        <span class="pull-right" data-ng-switch="isFavourite(item)">
            <span class="glyphicon glyphicon-star" aria-hidden="true" title="Remove as favourite" data-ng-click="toggleFavourite($event, item)" data-ng-switch-when="true"></span>
            <span class="glyphicon glyphicon-star-empty" aria-hidden="true" title="Select as favourite" data-ng-click="toggleFavourite($event, item)" data-ng-if="favouritesLeft()" data-ng-switch-default=""></span>
        </span>
