      ...
      <form id="create-video-form" class="clearfix" role="form" method="POST" action="{{ url('api/v1/projects/'.$project->id.'/videos') }}" v-on:submit="startLoading">
         ...
         <div class="form-group{{ $errors->has('url') ? ' has-error' : '' }}">
            ...
            <p class="help-block">
               ...
               @else
                  The video file of a <a href="{{route('manual-tutorials', ['videos', 'remote-videos'])}}">remote video</a> (e.g. <code>https://my-domain.tld/videos/file.mp4</code>) or on the BIIGLE server (e.g. <code>local://videos/file.mp4</code>). Supported video file formats are: MP4 (H.264) and WebM.
               @endif
            </p>
            @if($errors->has('url'))
