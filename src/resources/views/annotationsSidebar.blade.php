    ...
    <div class="ate-example-patches" data-ng-controller="AteExamplePatchesController">
        <div class="patch-container ng-cloak" data-ng-switch="isLoading()" data-ng-if="isEnabled() && hasLabel()">
            ...
            <div data-ng-switch-default="" data-ng-switch="hasPatches()">
                <img class="patch" src="{{ asset(config('thumbnails.empty_url')) }}" data-ng-src="{{ url('api/v1/annotations/') }}/@{{ patch }}/patch" data-fallback-src="{{ asset(config('thumbnails.empty_url')) }}" data-ng-switch-when="true" data-ng-repeat="patch in getPatches()" title="Example annotation for label @{{getLabelName()}}">
