<div class="panel panel-default" data-ng-controller="AnnotationSessionController" data-ng-class="{'panel-warning panel--editing': isEditing()}">
            ...
            <div class="row">
                <div class="form-group col-xs-6" data-ng-class="{'has-error':hasError('starts_at')}">
                    ...
                    <div class="input-group">
                        ...
                        <span class="input-group-btn">
                            <button type="button" class="btn btn-default" ng-click="openStartsAt()" data-ng-class="{'btn-danger':hasError('starts_at')}" title="Show date picker"><i class="glyphicon glyphicon-calendar"></i></button>
                        </span>
                    </div>
                ...
                <div class="form-group col-xs-6" data-ng-class="{'has-error':hasError('ends_at')}">
                    ...
                    <div class="input-group">
                        ...
                        <span class="input-group-btn">
                            <button type="button" class="btn btn-default" ng-click="openEndsAt()"  data-ng-class="{'btn-danger':hasError('ends_at')}" title="Show date picker"><i class="glyphicon glyphicon-calendar"></i></button>
                        </span>
                    </div>
            ...
            <div class="row">
                ...
                <div class="form-group col-xs-6" data-ng-class="{'has-error':hasError('users')}">
                    ...
                    <div class="form-control tag-list" readonly>
                        <span class="tag label label-default" data-ng-repeat="user in sessionUsers">
                            <span data-ng-bind="user.firstname"></span> <span data-ng-bind="user.lastname"></span> <button type="button" class="close" title="Remove @{{user.firstname}} @{{user.lastname}}" data-ng-click="removeUser(user)"><span aria-hidden="true">&times;</span></button>
                        </span>
                    </div>
                    <p class="help-block ng-cloak" data-ng-bind="getError('users')" data-ng-if="hasError('users')"></p>
                </div>
            </div>
            <div class="row">
    ...
    <ul class="list-group images-list ng-cloak">
            <li class="list-group-item session" title="@{{isEditing() ? 'Edit this annotation session' : session.name}}" data-ng-repeat="session in getSessions() | orderBy:dateComparator:true track by session.id" data-ng-if="hasSessions()" data-ng-class="{'session--active': isActive(session), 'list-group-item-info': newSession.id === session.id}" data-ng-click="isEditing() && editSession(session)">
                ...
                <div>
                    <span class="label label-default">@{{session.users.length}} user(s)</span>
