...
@section('manual-title') Remote transects @stop
...
@section('manual-content')
    <div class="row">
        ...
        <p>
            Typically the image files of transects are loaded through a network filesystem or even stored on the same machine that runs the BIIGLE application. If you request an image (e.g. in the annotation tool), the application receives the request, loads the image from the filesystem and returns the image file in the response.
        </p>
        <p>
        ...
        <p>
            If you want to revoke access to the images of your remote location, just turn your webserver off. Although BIIGLE will keep all information of the transect (annotations, etc.) BIIGLE users won't be able to access the original images any more.
        </p>
        <h3>How to secure a remote location</h3>
        <div class="panel panel-warning">
            <div class="panel-body text-warning">
                Note that every BIIGLE user who has access to the remote transect will always know and have access to your remote location as well (as long as it exists).
            </div>
        </div>
        <p>
        ...
        <p>
            It is also important to note, that HTTP basic auth credentials are always stored and sent in clear text. This basically means that basic auth is <strong>not</strong> more secure than the method with a long random directory name described previously! Every BIIGLE user who has access to the remote transect will see the basic auth credentials.
        </p>
        <h3>Drawbacks of remote transects</h3>
        <p>
            One drawback of remote transects obviously is the fact that you have to make the images quasi-publicly accessible. There is always the possibility of a BIIGLE user who has access to a remote transect to take the URL of the remote location and share it with a third party (but they could share their BIIGLE login credentials just the same, for that matter).
        </p>
        <p>
            A bigger drawback is that some of the features of BIIGLE require the images to be stored locally. This means that a few features are disabled for remote transects. The core functionality for annotations and reports is always available, though.
        </p>
    </div>
@endsection
