        ...
        @if ($editInstance)
            <form method="POST" action="{{ url("api/v1/federated-search-instances/{$editInstance->id}") }}">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-2">
                        <input type="submit" class="btn btn-success" value="Save" title="Save changes">
                    </div>
                </div>
                    ...
                    <div class="col-sm-5 col-sm-offset-7">
                        <div class="form-group">
                            @if ($editInstance->local_token)
                                ...
                                <input type="submit" class="btn btn-danger btn-block" value="Deny remote access" title="Deny the instance to retrieve the local search index">
                            @else
                                <input type="hidden" name="local_token" value="1">
                                <input type="submit" class="btn btn-success btn-block" value="Allow remote access" title="Allow the instance to retrieve the local search index">
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <form method="POST" action="{{ url("api/v1/federated-search-instances/{$editInstance->id}") }}">
                <div class="row">
                    @if ($editInstance->remote_token)
                        ...
                        <div class="col-sm-5">
                            <input type="submit" class="btn btn-danger btn-block" value="Disable indexing" title="Disable indexing of the remote search index">
                        </div>
                    @else
                        <div class="col-sm-7">
                        ...
                        <div class="col-sm-5">
                            <input type="submit" class="btn btn-success btn-block" value="Enable indexing" title="Enable indexing of the remote search index">
                        </div>
                    @endif
                </div>
                <input type="hidden" name="_redirect" value="{{ route('admin-federated-search', ['edit' => $editInstance->id]) }}">
        ...
        @else
            <form method="POST" action="{{ url('api/v1/federated-search-instances') }}">
                ...
                <div class="row">
                    ...
                    <div class=" col-sm-5 form-group{{ $errors->has('url') ? ' has-error' : '' }}">
                    ...
                    <div class="col-sm-2">
                        <input type="submit" class="btn btn-success" value="Connect" title="Connect the new remote instance">
                    </div>
                </div>
