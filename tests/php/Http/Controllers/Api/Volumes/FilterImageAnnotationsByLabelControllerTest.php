...
class FilterImageAnnotationsByLabelControllerTest extends ApiTestCase
{
    ...
    public function testFilters()
    {
        ...
        $this->get("/api/v1/volumes/{$id}/image-annotations/filter/label/{$l1->label_id}?shape_id[]={$s1->value}")
            ->assertExactJson([$a1->id => $image->uuid, $a2->id => $image->uuid]);
        ...
        $this->get("/api/v1/volumes/{$id}/image-annotations/filter/label/{$l1->label_id}?shape_id[]={$s2->value}&user_id[]={$u2->id}&union=0")
            ->assertExactJson([$a3->id => $image->uuid]);
        ...
        $this->get("/api/v1/volumes/{$id}/image-annotations/filter/label/{$l1->label_id}?shape_id[]={$s1->value}&user_id[]={$u1->id}&union=1")
            ->assertExactJson([$a1->id => $image->uuid, $a2->id => $image->uuid]);
