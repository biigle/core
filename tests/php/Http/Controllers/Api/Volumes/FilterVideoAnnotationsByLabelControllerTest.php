...
class FilterVideoAnnotationsByLabelControllerTest extends ApiTestCase
{
    ...
    public function testFilters()
    {
        ...
        $this->get("/api/v1/volumes/{$id}/video-annotations/filter/label/{$l1->label_id}?shape[]={$s1->value}")
            ->assertExactJson([$a1->id => $video->uuid, $a2->id => $video->uuid]);
        ...
        $this->get("/api/v1/volumes/{$id}/video-annotations/filter/label/{$l1->label_id}?shape[]={$s2->value}&user_id[]={$u2->id}&union=0")
            ->assertExactJson([$a3->id => $video->uuid]);
        ...
        $this->get("/api/v1/volumes/{$id}/video-annotations/filter/label/{$l1->label_id}?shape[]={$s1->value}&user_id[]={$u1->id}&union=1")
            ->assertExactJson([$a1->id => $video->uuid, $a2->id => $video->uuid]);
        ...
        $this->get("/api/v1/volumes/{$id}/video-annotations/filter/label/{$l1->label_id}?user_id[]={$u1->id}&user_id[]=-{$u1->id}&shape[]=1&union=1")
            ->assertExactJson([$a1->id => $video->uuid, $a2->id => $video->uuid, $a3->id => $video->uuid]);
