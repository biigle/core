...
class TransectImageLabelControllerTest extends ApiTestCase
{
    public function testFindLabel()
    {
        ...
        $this->doTestApiRoute('GET', "/api/v1/transects/{$tid}/image-labels/find/my");

        $this->beUser();
        $this->get("/api/v1/transects/{$tid}/image-labels/find/my");
        ...
        $this->get("/api/v1/transects/{$tid}/image-labels/find/label")
            ->seeJsonEquals([
                [
                    'id' => $label1->id,
                    'name' => $label1->name,
