...
class GetUsersWithAnnotationsVolumesTest extends ApiTestCase
{
    public function setUp(): void
    {
        ...
        $this->project->addUserId($this->user1, Role::editorId());
    ...
    {
        $this->beEditor();
        $this->get('api/v1/volumes/'.$this->imageVolume->id.'/users-with-annotations')->assertStatus(403);

        $this->be($this->user1);
        $response = $this->get('api/v1/volumes/'.$this->imageVolume->id.'/users-with-annotations')->assertStatus(200)->getContent();
        $expected = [['user_id' =>$this->user1->id, 'firstname' => $this->user1->firstname, 'lastname' => $this->user1->lastname]];
        ...
        $expected = [['user_id' =>$this->user2->id, 'firstname' => $this->user2->firstname, 'lastname' => $this->user2->lastname]];
        $this->assertEquals($expected, json_decode($response, true));
    }
}
