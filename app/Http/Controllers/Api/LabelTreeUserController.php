...
class LabelTreeUserController extends Controller
{
     ...
     * @apiParam (Required attributes) {Number} role ID of the role of the new member (admin or editor).
     *
    ...
    {
        $request->tree->addMember($request->input('id'), $request->input('role'));
     ...
     *
     * @apiParam (Attributes that can be updated) {Number} role New role of the member (admin or editor)
     *
    ...
    public function update(UpdateLabelTreeUser $request)
    {
        if ($request->filled('role')) {
            $request->tree->updateMember($request->member, $request->input('role'));
        }
