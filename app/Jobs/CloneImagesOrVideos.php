            ...
            ->with([
                'annotations' => fn ($q) =>
                    $q->chunkById(
                        $chunkSize,
                        fn ($chunk) =>
                        $chunk->whereIn('id', $usedAnnotationIds)->sortBy('id')
                    ),
                'annotations.labels' => fn ($q) =>
                    $q->chunkById(
                        $chunkSize,
                        fn ($chunk) =>
                        $chunk->whereIn('label_id', $imageAnnotationLabelIds)->sortBy('id')
                    ),
            ])
            ...
            ->with([
                ...
                'annotations.labels' => fn ($q) =>
                    $q->chunkById(
                        $chunkSize,
                        fn ($chunk) =>
                        $chunk->whereIn('label_id', $videoAnnotationLabelIds)->sortBy('id')
                    ),
            ])
